...
def runner(param):
	...
	path = os.path.join(save_path, file_name[:file_name.find(r'.')])
	path = os.path.join(path, str(iteration))
	VL = VL_ECOC(data, label, base_classifier=SVC, save_path=path, class_level_matrix_threshold=2, sample_level_matrix_threshold=2, feature_ratio=0.8, sample_ratio=0.8)
	accuracy, fscore, matthews, train_time, predict_time = VL.score()
	print(file_name[:file_name.find(r'.')]+' persistence record...')
	VL.persistence_record()
	print(file_name[:file_name.find(r'.')] + ' visualizing...')
	VL.visualization()
	return (accuracy, fscore, matthews, train_time, predict_time)
...
if __name__=='__main__':
	if len(sys.argv)>=2 and sys.argv[1]:
		...
		dict_name = 'result'
	data_path = r'./UCI_data'
	save_path = os.path.join(r'./', dict_name)
	file_list = file_names = [file for file in os.listdir(data_path)]
	result_collection = []
	k_fold = 20
	start_time = time.time()
	pool = Pool(20)
	for i in range(k_fold):
		...
		for file in file_list:
			param = [(file,data_path,save_path, i,)]
			result = pool.apply_async(runner, param)
			result_row.append(result)
		result_collection.append(result_row)
	pool.close()
	pool.join()
	accuracy_collection = [[0]*len(result_collection[0]) for i in range(len(result_collection))]
	fscore_collection = [[0] * len(result_collection[0]) for i in range(len(result_collection))]
	matthews_collection = [[0] * len(result_collection[0]) for i in range(len(result_collection))]
	train_time_collection = [[0] * len(result_collection[0]) for i in range(len(result_collection))]
	predict_time_collection = [[0] * len(result_collection[0]) for i in range(len(result_collection))]
	for i in range(len(result_collection)):
		for j in range(len(result_collection[0])):
		  accuracy_collection[i][j] = result_collection[i][j].get()[0]
		  fscore_collection[i][j] = result_collection[i][j].get()[1]
		  matthews_collection[i][j] = result_collection[i][j].get()[2]
		  train_time_collection[i][j] = result_collection[i][j].get()[3]
		  predict_time_collection[i][j] = result_collection[i][j].get()[4]
	accuracy_df = pd.DataFrame(accuracy_collection, columns=[file[:file.find('.')] for file in file_list])
	fscore_df = pd.DataFrame(fscore_collection, columns=[file[:file.find('.')] for file in file_list])
	matthews_df = pd.DataFrame(matthews_collection, columns=[file[:file.find('.')] for file in file_list])
	train_time_df = pd.DataFrame(train_time_collection, columns=[file[:file.find('.')] for file in file_list])
	predict_time_df = pd.DataFrame(predict_time_collection, columns=[file[:file.find('.')] for file in file_list])
	accuracy_save_path = os.path.join(save_path, 'accuracy.csv')
	accuracy_df.to_csv(accuracy_save_path)
	fscore_save_path = os.path.join(save_path, 'fscore.csv')
	fscore_df.to_csv(fscore_save_path)
	matthews_save_path = os.path.join(save_path, 'matthews.csv')
	matthews_df.to_csv(matthews_save_path)
	train_time_save_path = os.path.join(save_path, 'train_time.csv')
	train_time_df.to_csv(train_time_save_path)
	predict_time_save_path = os.path.join(save_path, 'predict_time.csv')
	predict_time_df.to_csv(predict_time_save_path)
	end_time = time.time()
	pool.terminate()
